...
const projects = [
   {icon: '⌘', title: 'Project title', body: 'A short, editable description for a student or club project.'},
...
const Home: NextPage = () => {
   ...
   return <><Head><title>McMaster Computer Science Society</title><meta name="description" content="A demo dashboard for the McMaster Computer Science Society." /><link rel="icon" href="/McMaster-Cybersociety-Logo.jpg" /></Head>
      ...
      <section id="updates" className="section-shell content-section"><div className="section-heading"><p className="eyebrow">01 / Stay in the loop</p><h2>Announcements</h2><p>Swap these demo cards with your latest society news.</p></div><div className="announcement-grid">{announcements.map((item) => <article className="announcement-card" key={item.title}><div><span className="tag">{item.tag}</span><span className="date">{item.date}</span></div><h3>{item.title}</h3><p>{item.body}</p><a href="#contact">Read update <span>→</span></a></article>)}</div></section>
      <section id="projects" className="section-shell content-section projects-section"><div className="section-heading"><p className="eyebrow">02 / Made at Mac</p><h2>Project showcase</h2><p>A simple gallery template for student and club work.</p></div><div className="project-grid">{projects.map((project, i) => <article className="project-card" key={project.icon}><div className={`project-art art-${i + 1}`}><span>{project.icon}</span><small>YOUR PROJECT<br />IMAGE / PREVIEW</small></div><div className="project-content"><p className="card-label">Featured project</p><h3>{project.title}</h3><p>{project.body}</p><a href="#contact">View project <span>↗</span></a></div></article>)}</div></section>
      <section id="ctf" className="section-shell content-section ctf-section"><div className="section-heading"><p className="eyebrow">03 / Capture the flag</p><h2>CTF central</h2><p>Keep score, find your next competition, and build your team.</p></div><div className="ctf-grid"><article className="ranking-card"><p className="card-label">Recent rankings</p><ol><li><span>1</span><b>byte_bandits</b><em>4,920 pts</em></li><li><span>2</span><b>root_access</b><em>3,840 pts</em></li><li><span>3</span><b>mcmaster_cs</b><em>3,275 pts</em></li></ol><a href="#contact">See full rankings <span>→</span></a></article><article className="upcoming-card"><p className="card-label"><span className="status-dot" /> Coming up</p><h3>MapleCTF 2026</h3><p>Oct 18–20 · Online</p><div className="team-callout"><span>◎</span><div><b>Looking for a team?</b><p>Tell us what you&apos;re into and we&apos;ll help you find one.</p></div></div><a className="button button-primary" href="#contact">Register interest <span>→</span></a></article></div></section>
      <section id="resources" className="resource-band"><div className="section-shell"><div className="section-heading"><p className="eyebrow">04 / Keep learning</p><h2>Resources for the curious</h2></div><div className="resource-grid">{resources.map((resource, i) => <a className="resource-card" href="#contact" key={resource.title}><span>0{i + 1}</span><h3>{resource.title}</h3><p>{resource.text}</p><b>Explore <i>→</i></b></a>)}</div></div></section>
